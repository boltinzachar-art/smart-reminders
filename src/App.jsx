...
const useDataStore = (userId, isOnline, toast) => {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      if (tRes.data) {
         ...
         const map = new Map(); tRes.data.forEach(x => map.set(x.id, x)); local.forEach(x => map.set(x.id, x));
  ...
  return { tasks, lists, templates, setLists, setTemplates, modifyTask };
};
...
const TaskItem = ({ task, onAction, viewMode, selectionMode, isSelected, onSelect, onEdit }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} {...(selectionMode ? {} : { ...attributes, ...listeners })}>
      ...
      <div style={contentStyle} onTouchStart={handleTouch.start} onTouchMove={handleTouch.move} onTouchEnd={handleTouch.end} onClick={handleMainClick} className={`relative z-10 group w-full bg-white rounded-xl p-4 shadow-sm flex items-start gap-3 transition-colors ${isCompleting ? 'bg-gray-50' : ''} ${isDragging ? 'shadow-xl ring-2 ring-blue-500/20' : ''}`}>
        <button onPointerDown={e=>e.stopPropagation()} onClick={(e)=>{
            ...
            if(selectionMode) onSelect(task.id);
                ...
                else { setIsCompleting(true); setTimeout(() => { onAction('complete', task); setIsCompleting(false); }, 2000); }
            }
        ...
        <div className="flex-1 min-w-0 pt-0.5 overflow-hidden">
          ...
          <div className="flex items-center flex-wrap gap-2 mt-2 h-6 overflow-hidden">
            ...
            {task.type !== 'reminder' && (
              <button onPointerDown={e=>e.stopPropagation()} onClick={(e) => {
                  e.stopPropagation();
                  const txt = encodeURIComponent(task.description||'');
                  const sub = encodeURIComponent(task.title);
                  if(task.type==='copy') navigator.clipboard.writeText(task.description||task.title);
                  else if(task.type==='whatsapp') window.open(`https://wa.me/?text=${txt}`);
                  else if(task.type==='email') window.open(`mailto:?subject=${sub}&body=${txt}`);
                  else if(task.type==='call') window.open(`tel:${task.description}`);
                  else if(task.type==='web_search') window.open(`https://google.com/search?q=${sub}`);
              }} className="text-xs font-bold text-gray-600 bg-gray-100 px-2 py-1 rounded flex items-center gap-1 hover:bg-gray-200 transition">
                  {task.type === 'whatsapp' ? <MessageCircle size={12}/> : task.type === 'email' ? <Mail size={12}/> : task.type === 'copy' ? <Copy size={12}/> : <Search size={12}/>}
                  {task.type === 'whatsapp' ? 'WhatsApp' : task.type === 'email' ? 'Email' : task.type === 'copy' ? 'Копия' : 'Действие'}
              </button>
            )}
          </div>
        </div>
...
const MainApp = () => {
  ...
  const { tasks, lists, templates, setLists, setTemplates, modifyTask } = useDataStore(userId, isOnline, toast);
  
  // UI States
  const [selectionMode, setSelectionMode] = useState(false);
  const [selectedIds, setSelectedIds] = useState(new Set());
  const [activeModal, setActiveModal] = useState(null); // 'task', 'list', 'templates'
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-[#F2F2F7] text-black font-sans flex flex-col overflow-hidden">
      ...
      <div className="px-4 pt-2 pb-2 bg-[#F2F2F7] sticky top-0 z-20 flex flex-col gap-2">
         ...
         {/* LIST TITLE (IF NOT HOME) */}
         {view !== 'home' && (
             <div className="flex justify-between items-end pb-2">
                 <h1 className="text-3xl font-bold text-blue-600">
                    {lists.find(l => l.id === view)?.title || (view === 'today' ? 'Сегодня' : view === 'upcoming' ? 'Запланировано' : view === 'all' ? 'Все' : view === 'flagged' ? 'С флажком' : view === 'trash' ? 'Корзина' : 'Выполнено')}
                 </h1>
             </div>
         )}
      </div>
         ...
         {(view !== 'home' || search) && (
             <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(e) => {
                 ...
                 if (active.id !== over.id) {
                    ...
                    // Update local state to reflect UI change immediately (simplified for brevity, ideally update 'tasks')
                 }
             }}>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                  ...
                  <div className="bg-white rounded-xl p-4 shadow-sm space-y-3 border border-purple-100">
                      <div className="flex items-center justify-between">
                          ...
                          <button onClick={() => { /* SAVE TEMPLATE LOGIC HERE */ }} className="text-xs bg-gray-100 px-2 py-1 rounded flex items-center gap-1"><BookmarkPlus size={14}/> В шаблон</button>
                      </div>
                      ...
                      {aiResult && <div className="bg-gray-50 rounded-lg p-3 border border-gray-200 animate-in fade-in"><div className="text-sm text-gray-700 mb-3 whitespace-pre-wrap">{aiResult}</div><button onClick={() => { setNewT({...newT, description: aiResult}); }} className="w-full bg-black text-white text-sm font-bold py-2 rounded-lg flex items-center justify-center gap-2"><ArrowDown size={14}/> Вставить</button></div>}
                  </div>
                  
                  <div className="bg-white rounded-xl overflow-hidden shadow-sm space-y-[1px] bg-gray-100">
                      <div className="bg-white p-3.5 flex justify-between items-center"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-red-500 flex items-center justify-center text-white"><CalendarIcon size={18} fill="white" /></div><span className="text-[17px] text-black">Дата</span></div><IOSSwitch checked={newT._hasDate} onChange={v => setNewT({...newT, _hasDate: v})} /></div>
                      {newT._hasDate && <div className="bg-white px-4 pb-3 animate-fade-in-ios"><input type="date" value={newT._dateVal} onChange={e => setNewT({...newT, _dateVal: e.target.value})} className="w-full p-2 bg-gray-100 rounded text-blue-600 font-semibold outline-none text-right" /></div>}
                      <div className="bg-white p-3.5 flex justify-between items-center"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-blue-500 flex items-center justify-center text-white"><Clock size={18} fill="white" /></div><span className="text-[17px] text-black">Время</span></div><IOSSwitch checked={newT._hasTime} onChange={v => setNewT({...newT, _hasTime: v})} /></div>
                      {newT._hasTime && <div className="bg-white px-4 pb-3 animate-fade-in-ios"><input type="time" value={newT._timeVal} onChange={e => setNewT({...newT, _timeVal: e.target.value})} className="w-full p-2 bg-gray-100 rounded text-blue-600 font-semibold outline-none text-right" /></div>}
                  </div>
      ...
      {activeModal === 'listSettings' && (
          <Modal onClose={() => setActiveModal(null)}>
             <div className="flex justify-between items-center px-4 py-4 border-b border-gray-200"><button onClick={() => setActiveModal(null)} className="text-blue-600 font-medium">Закрыть</button><h3 className="font-bold text-black">Списки</h3><button onClick={() => { /* Create List Logic */ }} className="text-blue-600 font-bold"><Plus size={24}/></button></div>
             <div className="flex-1 overflow-y-auto p-4 space-y-2">
                 {lists.map(l => (
                     <div key={l.id} className="relative overflow-hidden rounded-xl mb-2 bg-white p-4 flex items-center gap-3 shadow-sm">
                         <ListIcon size={16} className="text-blue-600" /><span className="flex-1 text-[17px] font-medium text-black">{l.title}</span>
                     </div>
                 ))}
             </div>
          </Modal>
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-2">
                 ...
                 {templates.map(t => (
                     <button key={t.id} onClick={() => { 
                         setNewT(p => ({ ...p, title: t.title, description: t.description, type: t.type, url: t.url||'' })); 
                         setActiveModal('task'); 
                     }} className="w-full bg-white p-3 rounded-xl text-left shadow-sm">
                         <div className="font-bold text-black">{t.title}</div><div className="text-xs text-gray-500">{t.description}</div>
                     </button>
                 ))}
             </div>
          </Modal>
...
class ErrorBoundaryWrapper extends React.Component {
  render() { return <ErrorBoundary><ToastProvider><MainApp /></ToastProvider></ErrorBoundary>; }
}
