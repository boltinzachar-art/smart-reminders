...
// Добавил GripVertical обратно
import { Plus, Search, ExternalLink, RefreshCw, ChevronRight, CloudOff, RotateCcw, Trash2, GripVertical } from 'lucide-react';
...
const SortableTaskItem = ({ task, completeTask, deleteTask, restoreTask, performAction, formatTime, isOverdue, isTrashMode }) => {
  ...
  return (
    <div 
        ...
        style={style} 
        className={`
            group w-full bg-white rounded-xl p-3 shadow-[0_1px_2px_rgba(0,0,0,0.05)] 
            flex items-start gap-2 touch-manipulation transition-all duration-500
            ${isFlashing ? 'bg-gray-50' : 'bg-white'} 
            ${isDragging ? 'shadow-xl ring-2 ring-blue-500/20' : ''}
        `}
    >
       {/* 1. РУЧКА ДЛЯ ПЕРЕТАСКИВАНИЯ (Grip Handle) */}
       {!isTrashMode && (
           <div 
             {...attributes} 
             {...listeners} 
             className="mt-1 p-1 text-gray-300 cursor-grab active:cursor-grabbing touch-none"
           >
...
const App = () => {
  ...
  // СЕНСОРЫ (Убрали задержку delay, так как теперь есть явная ручка)
  const sensors = useSensors(
    useSensor(PointerSensor),
    useSensor(TouchSensor, { 
        // activationConstraint убрали или сделали минимальным, 
        // потому что мы тянем за конкретную иконку, скроллу это не мешает
